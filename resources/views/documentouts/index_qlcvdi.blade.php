    ...
    <main class="col-md-9 shadow rounded">
      <div class="d-flex justify-content-between flex-wrap  align-items-center border-bottom">
        <div class="card mb-2 col-md-12">
          <div class="card-body">
              <a class="btn btn-danger" href="{{ route('qlcvdith')}}">Công văn trễ hạn</a>
          </div>
      </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              @cannot('nguoidungthuong')
              <td><a href="{{ route('cong-van-di.edit', $documentout->id)}}" class="btn btn-primary">Edit</a></td>
              <td>
                <form action="{{ route('cong-van-di.destroy', $documentout->id)}}" method="post">
                  @csrf
                  @method('DELETE')
